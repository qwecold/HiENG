...
export function AuthForm() {
  const { signInWithGithub, loginAsGuest } = useAuth()
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full px-6 py-3 bg-muted text-muted-foreground font-medium rounded-lg hover:bg-muted/80 active:bg-muted/60 transition-colors"
          >
            Войти как гость
          </button>
          <p className="text-xs text-muted-foreground text-center">
            Данные гостя сохраняются только в этом браузере
          </p>
        </div>
      </div>
    </div>
